...
**적용 상태:** 새 작업은 이 규칙을 따른다. 2026-09-11에 기존 코어의 `src/` 이동과 예제·시험·문서 배치를 적용했으며, 마지막 절의 표는 전환 이력이다. 개발 도구·ROS 컨테이너 구성 상태와 실행 결과는 `README.md`와 `docs/development.md`에서 확인한다. 이 문서의 작성 자체가 하드웨어 구매, 실물 연결 또는 로봇 기능 구현을 뜻하지 않는다.
...
아래는 **목표 위치 규칙**이다. 구현된 항목은 이 배치를 따르며 마지막 절에 전환 이력을 남겼다. 아직 구현하지 않은 기능의 빈 폴더·빈 클래스·성공하는 척하는 테스트를 미리 만들지 않는다.
...
Python 배포 패키지명은 `forklift-core`, import 경로는 `forklift_core`로 통일한다. 배포명 전환도 2026-09-11에 적용했다. root의 `src/`는 Python 소스이고, `ros2/src/`는 colcon 패키지 위치다. 서로 복사하거나 같은 패키지를 두 번 두지 않는다.
...
비공개 함수는 다른 형제 모듈에서 직접 가져오지 않는다. 패키지 안에서 공유할 검증은 `_validation.py`의 명시적인 내부 API로 두고, 패키지 밖에서는 사용하지 않는다. 공유 입력 검증은 `src/forklift_core/_validation.py`에 두며 센서 모듈도 이 내부 API를 사용한다.
...
- 초기 RED 확인에 썼던 시험의 동적 import는 구조 전환 때 모듈 상단 import로 바꿨다. 특별한 이유가 없는 테스트는 일반적인 모듈 상단 import를 사용한다.
...
기본 개발 설치는 대상 Python 환경에서 `python -m pip install -e '.[dev]'`로 통일한다. `sys.path.insert`, 관행적인 전역 `PYTHONPATH`, 전역 pip 설치를 표준 실행 절차로 삼지 않는다. 패키징을 바꾸면 실제 설치와 import 경로를 검증한다.
...
새 문서는 도구 이름을 경로에 넣지 않는다. 기존 설계·계획 문서는 2026-09-11에 각각 `docs/design/`와 `docs/plans/`로 이동하고 관련 링크도 갱신했다. 과거 검증 결과·원시 기록의 의미는 바꾸지 않는다.
...
**2026-09-11 구조 적용 완료. 아래 표는 전환 이력이다.** 코어 함수의 동작을 유지하며 아래 변경을 구조 정리 작업 하나로 적용했다. 환경별 실행 절차와 남은 검증은 [개발 환경](docs/development.md)을 따른다.
...
기존 코어 64개 시험의 행동을 유지한다. 포맷/정적 검사와 editable·wheel 설치 경로는 환경별로 확인한다. 미구현 기능의 빈 폴더는 생성하지 않는다. 전환 후 실행 명령은 `README.md`와 `docs/development.md`를 따른다.

...
## 형상 일반화 — 완료 (2026-09-18, STAGE A-D)

아래 절 전체는 2026-09-17 작성 당시의 미완 상태 기록이다. 구현은 STAGE A→D 네 커밋으로 끝냈다 —
`627850c`(핵심 형상 재설계·`target_insertion_depth_m` 단일화), `1bc8b03`(실행기·대규모 검증기
배선, `--pallet-geometry` 필수화·1mm 대조), `7f43037`(정사각 T11 90°/180° 오정렬을 잡는 22박스
이름별 대조), `18649a8`(`DEFAULT_TRANSPORT_CLEARANCE_M`·근접-여유 변이 회귀·YAML 해시·22박스
assertion 일반화). 각 단계 Codex 적대적 검토를 거쳤고 전체 회귀는 898→927→931→933 passed(+1
skip 유지)로 늘었다. 아래 서술 중 "미완"·"구현자가 정할 것" 표현은 **당시 기준**이며, 실제 구현이
바꾼 지점은 위 커밋 메시지에 정정해 두었다(예: `:602` 의 F 대입 오류, `pallet_boxes()` 위치 이동).
T11 실행 자체는 여전히 표준(235/350mm) 미확정으로 막혀 있다 — 이건 코드가 아니라 사용자 결정이
필요한 별개 항목이다.

## 형상 일반화 (2026-09-17 작성 당시 기록, 3 라운드 교차검증)
...
| `:602` | `0.95` (`base_link` → 포크끝) | `F − \|후축 오프셋\|` — ⚠️ **F 자체가 아니다.** `final_base` 는 `base_link` 이지 후축이 아니다(`:438` `base, q = robot.get_world_pose()` → `rear = base − 0.34·forward` 로 후축을 매번 따로 구한다). F 는 **후축 기준**이므로 `base_link` 위치에 그대로 더하면 후축→`base_link` 오프셋(0.34)만큼 어긋난다. 스테이지 B 구현에서 `F` 그대로 대입했다가 실측 대조로 0.34 m 오차를 잡아 정정했다(2026-09-18) |
...
prior match"** 라고 적어 두었다.

**결정 (2026-09-18, 사용자): (나) — YAML 의 내부 박스 배치까지 URDF 와 대조한다.**

⚠️ **런타임 `InsertionGeometry` 는 안전망이 아니다 — 이 결정의 근거이지, 대안이 아니었다.**
`block_widths_m`(`:34`)·`block_depth_m`(`:35`)이 두 축 모두 80·210·80·210·80 = 660 mm 로
**동일**하다 — 이 T11 은 실제로 사방 삽입(four-way) 설계라 블록 그리드는 90° 회전에 대해
대칭이다. 즉 **잘못 90° 돌아간 URDF 도 양쪽 면에 포켓이 존재**하므로, 삽입 시도 시 포켓이
탐지된다는 사실만으로는 회전 오류를 못 잡는다. (가)를 골랐다면 이 대칭성 때문에 오정렬이
조용히 통과했을 것이다.

**대조 대상 — 블록 위치만으로는 부족하다. 판별자는 하나가 아니라 셋이다** (Codex 적대적 검토,
2026-09-18, `tools/build_pallet_model.py:41-78` 재계산으로 확인). `pallet_boxes()`가 만드는 22개
박스의 크기(x×y×z)를 실제로 뽑으면:

| 부품 | 개수 | 크기(mm) | 장축 | 90° 회전 대칭인가 |
|---|---:|---|---|---|
| 하부 보드 | 3 | 660×80×15 | x | 아니다 |
| 블록 | 9 | 80×80×45 | 없음(정사각) | **맞다(이름 없는 형상 기준)** — 이름별 중심까지 보면 판별자다 |
| 스트링거 | 3 | 80×660×15 | **y** | 아니다 |
| 상부 보드 | 7 | 660×82.5×15 | x | 아니다 |

**"블록은 회전 불변"은 이름 없는 그리드 형상에 한정된 말이다.** 직접 재계산으로 확인했다 —
`pallet_boxes()`가 만든 22개를 이름째로 90° 스왑(중심 (x,y)→(−y,x), 크기 (sx,sy)→(sy,sx))하면
**21개가 달라진다**(블록 9개 중 8개 포함 — 중심 블록만 (0,0)이라 안 바뀐다). 이름을 지키는 대조라면
블록도 이미 판별자다. 하부 보드·스트링거·상부 보드는 그 위에 **장축까지** 있어 이름 없이도
판별된다. 대조는 **22개 박스 전체의 이름·중심(x,y,z)·크기(x,y,z)**를 YAML 유도값과 비교한다.
"하부 보드 하나만 보면 된다"는 판단은 폐기한다.

⚠️ **하부 보드 지지 연속성 검사는 안정성 보장이 아니다.** 이 대조가 통과해도 M6 실측 지지·하중
시험 전까지는 물리적 안정성이 검증된 것이 아니다 — 여기서 하는 것은 **모델 조립이 의도한 배치와
일치하는지**뿐이다.

⚠️ **이 대조는 이름·위치·치수의 불일치를 잡지만, 물리적 앞뒤 모호성 자체를 없애지는 못한다.**
직접 재계산: 22개를 이름째로 180° 회전(중심 (x,y)→(−x,−y))하면 **18개가 달라진다** — 즉 이름
붙은 대조는 180° 로 뒤집어 조립된 URDF 도 정상적으로 거부한다. 하지만 **이름을 떼고 순수 형상
집합만 보면 22개 박스 전체가 180° 회전에 대해 대칭**이다 — 동일 치수의 부품들이 xy 중심에 대해
대칭 쌍으로 배치되고, 가운데 부품(중심 블록·중앙 스트링거·중앙 상부 보드)도 자체적으로 180°
회전에 대해 불변이기 때문이다. 그러므로 이 대조가 잡는 것은 "조립이 YAML 이 의도한 이름-위치
대응과 다르다"이지, "이 물리적 형상에서 어느 쪽이 진짜 진입면인가"가 아니다 — 형상만으로는
앞뒤를 구별할 수 없는 경우가 있다는 뜻이고, 그 구별은 여전히
`PalletSite.yaw_rad`/`InsertionGeometry` 가 진다(둘 다 자세를 **가정**해서 검사할 뿐, 형상에서
진입면을 **추론**하지는 않는다).

**형상 YAML 의 내용 해시를 결과에 남긴다.** [ADR 0004 D2](../decisions/0004-simulation-engine-and-insertion-depth.md)
가 전이적 소스와 설정의 해시를 요구하는데, `asdict(geometry)` 는 `source_provenance`·
`geometry_version` **필드값**은 남겨도(`pallet_geometry.py:20-21`) **원본 YAML 의 정확한 바이트
내용·주석·파일 해시는 보존하지 않는다** — 같은 provenance 문자열을 쓰는 서로 다른 YAML 판본을
구별 못 한다. T11 해석이 미확정인 동안에는 특히 그렇다.
...
| T11 90° 오정렬 거부 | ⚠️ **`rpy` 를 90° 로 바꾼 fixture 를 쓰지 않는다** — `insertion_geometry.py:44` 가 `rpy != 0` 을 이미 무조건 거부하므로(`"Source collision boxes must have zero local rpy"`), 새 배치 검사가 없어도 그 이유만으로 통과해 시험이 무의미해진다. 대신 **`rpy=0` 을 유지한 채 이름은 그대로 두고 중심을 (x,y)→(−y,x), 크기를 (sx,sy)→(sy,sx) 로 바꾼** fixture 를 쓴다 — 직접 재계산 결과 22개 중 21개(블록 8개 포함, 중심 블록만 제외)가 원래 값과 달라지므로 이름별 대조가 없으면 통과하고 있으면 거부되는 변이다. 세 보드군만 90° 로 바꾸고 블록은 그대로 둔(치수는 다 맞지만 조립이 틀린) 변형도 별도로 추가한다 — 이건 "블록 그리드 대칭성만으로 충분하다"는 착각을 잡는 시험이다 |

...
2. `u1`·`u2` 와 `upper_ok = min(u1,u2) >= min_band_points` 를 기록한다.
   - **점 집합은 개구 위 *부피* 다**(평면 inlier 가 아니다). 조건: `height_m - deck_top_m <= z <= height_m + 0.30`, 평면 기준 깊이 `-0.25 <= depth <= overall_depth_m`, 횡방향은 각 개구 구간. **적재 팔레트 대응 때문에 이렇게 정한다**(§C-8). 평면 inlier 로 한정하면 화물이 앞으로 150 mm 만 튀어나와도 근거리에서 상판이 가려져 실패한다.
     v1 100 장면 회귀는 **평면 방식과 완전히 동일**하다 — `s056` 한 장면만 다르고 좌표 불변, `summarize` 불변(직접 재측정). 값은 커진다(빈 EPAL 6 2.0 m 에서 2674 → 7934).
...
**해법: 상한 증거를 평면이 아니라 부피로 센다**(§B 규칙 2). 개구 위 공간에 무엇이든 있으면 — 그것이 상판이든 그 위의 화물이든 — 증거로 친다.

| 사례 | 평면 inlier(기존) | **부피(채택)** |
|---|---|---|
| 돌출 150 mm | 2.0·2.5 m 실패 | **전 거리 검출** |
| 돌출 200 mm | 3.0 m 까지 실패 | 2.5·3.0 m 검출, 2.0 m 는 `pocket_occluded` |
| 돌출 300 mm | 실패 | **여전히 실패** |
| 빈 팔레트 | 검출 | 검출 |
| **맨 블록 아홉 개** | **거부** | **거부**(u = 0) |
| 블록 + 상자(전폭) | 통과 | 통과 (기존과 동일, 신규 아님) |
| 앞면 개방 구조 | 통과(§C-2) | 통과 |
| v1 100 장면 | `s056` 만 | **완전히 동일** |

**즉 얻는 것만 있고 잃는 것이 없다.** 다만:

- **돌출 300 mm 는 여전히 실패한다.** 이건 증거 정의 문제가 아니라 **평면 선택** 문제다 — 화물의 앞면이 지배 평면이 되어 팔레트 전면을 못 찾는다(`no_front_plane`). `max_plane_candidates = 3` 과 s009 의 점 선점 결함(§H)이 여기서 겹친다.
- **음성들을 부피 정의로 다시 재야 한다.** 위 표에서 확인한 것은 맨 블록·전폭 상자·개방 구조뿐이고, §C-1 의 비대칭 누출 조합은 부피 정의로 아직 안 쟀다.

**측방 돌출과 화물 높이는 무관하다 — 측정했다.** 한쪽 0 / 50 / 100 / 200 / 400 mm(화물 폭 800 → 1600 mm)에서 판정과 `u` 가 사실상 동일하다. 화물은 개구 *위*에 있으므로 옆으로 얼마나 나가든 개구 위 증거가 안 바뀐다. 높이도 100 / 300 / 600 / 1000 mm 전부 동일하다(부피 창이 +0.30 에서 잘리므로 그 위는 애초에 안 센다). **이전 판에서 이 둘을 "안 쟀다"고 나열한 것은 물리적으로 무의미한지 따지지 않고 적은 것이라 삭제한다.**

...
**이 문제는 s009 와 같은 뿌리로 보인다**(§H — 거부될 후보 평면이 정답 평면의 점을 먼저 소비하는 구조적 결함). 적재 팔레트가 그 결함을 한 장면짜리 사례에서 **상시 상태**로 만든다. **s009 를 더 미루는 근거가 약해졌다.**

아직 안 쟀다: 이 사각지대가 `plane_inlier_m` 을 바꾸면 어디로 옮겨가는지, `max_plane_candidates` 를 늘리면 사라지는지, 다른 팔레트 형상에서 밴드 폭이 어떻게 되는지.

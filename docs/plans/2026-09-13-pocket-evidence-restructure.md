# 포켓 증거 구조 변경과 M2 재평가 계획 (v9)

> 상태: **v9 — 독립 검토 24회 반영.** v1 `22e967b`, v2 `1644063`, v3 `f633b7a`, v4 `41d4e52`, v5 `3154aea`, v6 `e3d00dd`, v7 `a25f2eb`, v8 `32a22e3`.
...
**내 권고: (c).** §D-1b 가 근거다 — 캡처가 답하려던 질문(EPAL 6 양성 검출)은 **커밋된 80 자세로 이미 답했고 100 %·p95 6.3 mm 로 통과**한다. 남는 미답은 실센서에서 44 mm 대역이 읽히는가(§C-4)인데 **합성 장면을 더 찍어서는 답이 안 나온다.** 그 사이 로드맵은 다음 주를 H0/H1 로, 그다음 주를 M3 추적으로 잡아 두었고 M3 는 실물 없이 검증 가능한데 코드가 0 줄이다. 캡처는 H0 가 마운트를 확정한 뒤에 하면 자세 범위(§A ②)도 같이 정해진다.
...
# B0. s009 수정 이후 전면 재측정 (2026-09-14, HEAD `65ef9a0`)
...
2. `u1`·`u2` 와 `upper_ok = min(u1,u2) >= min_band_points` 를 기록한다.
   - **점 집합은 평면 inlier 다.** 2.5 m 에서 1033/1060 이다.
     ...
     **부피 방식이 사는 것은 24 칸 중 5 칸뿐**이다(돌출 150 mm 의 2.0·2.5 m, 200 mm 의 2.5·3.0 m). 나머지는 평면 방식과 동일하고, **§C-8 스스로 150 mm 돌출을 "현실적인 적재가 아니다" 라고 적었다.** 비현실적인 적재 5 칸을 사려고 계획 자신의 `negative_block_row` 계열에서 17/18 을 내주는 거래다.
   ...
   - *(폐기)* ~~z 상한을 `height_m + plane_inlier_m` 로 둔다~~ — 부피 정의의 `+0.30` 상한이 이를 대체한다. 아래 측정 기록은 남긴다.
...
**규칙 4 자체가 어디에서도 측정된 효과가 없다.** 카탈로그 자세 80 개에서도 규칙 4 와 z 상한을 네 조합으로 껐다 켰을 때 **판정이 한 장면도 안 바뀐다**(전부 70/80). v1 100 장면과 EPAL 6 125 자세 격자 모두에서 `upper_ok` 1순위 정렬과 순수 점수 정렬의 결과가 **0 건 차이**다. 그런데도 진단 수선 비용을 내고 `baseline.md:68` 을 깨뜨린다. **미측정 안전장치로 남긴다고 적는다.**
...
**그러나 한쪽에만 상한 증거가 있는 팔레트 아닌 구조는 여전히 샌다.** 블록 아홉 개 + 한쪽 절반 위에 얹힌 상자(앞면 정렬) + 반대쪽 개구 앞 장애물. 독립 재현 3건: 60 조합에서 14 건, 72 조합에서 24 건, **48 조합에서 34 건(71 %)** `invalid`(폭 불일치 19, `pocket_occluded` 15). 조합 정의가 달라 비율을 그대로 비교할 수는 없지만 **누출은 계획이 적은 것보다 크다.** 세 쓸이 모두에서 `valid` 는 0 이므로 **기하를 싣는 위양성은 없다.** **HEAD 는 두 쓸이 모두에서 전부 깨끗한 true negative** 다. *(v7 의 "60 중 38, 그 중 16 이 pocket_occluded" 는 재현되지 않았고 조합 정의도 안 적혀 있었다. 누출의 존재와 HEAD 가 절대 안 샌다는 것은 확인됐다.)* 음성 범주에서 `invalid` 는 `false_positive_rate` 에 안 잡히고 예산 없는 `counts.*.invalid` 로 간다(`evaluation.py:101-103, 170-173`). 이 누출은 얹힌 구조가 평면 inlier 대역(20 mm) 안에 있을 때만 생기고 19 mm 이상 물러나면 사라진다. **계획된 140 장면 중 이걸 잡을 장면은 하나도 없다.**
...
**이 표는 합성 리그의 카메라로 잰 것이다.** `sim/gazebo/scenes/catalogue_v1.yaml:6-9` 는 640×480·HFOV 1.204 rad(69°)·(0.75, 0, 0.5)이고, 확정 센서 D435i 는 약 87°×58°·848×480 또는 1280×720 이다. `fx`·`fy` 가 둘 다 달라 **행 수도 여유도 다른 카메라의 수다.** 같은 이유로 §B 의 유령 시험 자세 `y = 0.80` 도 리그의 반화각 34.5° 에서 나온 값이라 D435i(약 43.5°)에서는 옮겨간다. **H2 에서 실제 해상도·화각으로 이 표를 다시 뽑기 전까지 1.7× 를 인용하지 않는다.**

그리고 실패 기전이 잡음 산포가 아닐 수 있다. 4 m 에서 대역이 약 7 px 이면 **스테레오 상관 창과 비슷하거나 작아** 대역의 모든 픽셀이 아래의 빈 포켓과 깊이 불연속을 이룬다 — D435i 의 좌우 일관성 필터가 가장 공격적인 조건이다. **방향은 fail-safe 다**(잡음은 상판 재료를 만들지 않으므로 거부로 나타난다). 예외는 팔레트 **뒤** 화물의 foreground dilation 과 포켓 내부 IR 다중경로이고, 둘 다 적재·랙 상황이라 §C-2 와 겹친다.

**그런데 잡음을 실제로 넣어 보니 `u` 문턱은 한 번도 구속 조건이 되지 않았다.** σ = k·d²(3 m 에서 5/10/20 mm) + 3 % 픽셀 결측, 조건당 25 시행, 총 1584 회:
...
**따라서 v8 이 적은 "44 mm 대역 대 D435i 깊이 오차가 이 규칙의 생사"는 측정으로 지지되지 않는다.** 생사를 쥔 것은 `max_plane_residual_m` 이고 그것은 Task 8 의 튜닝 대상이다. 다만 **잡음은 §C-2 를 전혀 갈라주지 않고**(flush 개방 구조의 `u`·지지대·판정이 모든 잡음 수준에서 EPAL 6 와 사실상 동일), **4.5 m 는 σ 10 mm 만 넣어도 0/15 검출**이라 `range_max_m` 5.0 뿐 아니라 **4.5 m 도 사실상 도달 불가**다.
...
**해법이라고 적었던 것을 철회한다.** v9 는 상한 증거를 부피로 세는 것으로 이 문제를 풀었다고 적었으나, **그 변경은 기하를 싣는 위양성 17/18 을 만든다**(§B 규칙 2). 사는 것은 24 칸 중 5 칸이고 그 5 칸은 이 절 스스로 "현실적인 적재가 아니다" 라고 적은 돌출량이다. **거래가 성립하지 않는다.**
...
### C-9. 앞쪽 돌출 25~35 mm 에 사각지대가 있다 — 통상 적재 범위다
...
후보 수를 늘려도 전혀 안 변한다는 것이 점 선점 가설을 배제한다. 화물 앞면과 팔레트 앞면의 간격이 inlier 대역과 비슷해지면 **두 면이 하나의 평면 적합으로 뭉치고** 그 평면에는 개구가 없다.

**중요: 대역을 줄여도 밴드가 없어지지 않고 파라미터에 비례해 옮겨간다.** 20 mm 일 때 25~35 mm, 8 mm 일 때 20 mm 에서 실패가 나타난다. 즉 **"앞면에서 inlier 대역의 몇 배 거리에 또 다른 큰 평면이 있으면 깨진다"** 는 구조적 성질이고, 파라미터 조정으로는 위치만 바꾼다.
...
**새로 생기는 진짜 비용은 하나다:** §C-2 의 앞면 개방 구조를 4 mm 오차로 `valid` 로 내는 것 — 이 계통에서 변이 C 가 **기하를 실어 보내는** 유일한 위양성이다. 나머지는 버킷 라벨이고, §C-3 의 머리 위 가림은 HEAD 도 잃는다.
...
|---|---|---|---|
| positive | 60 | **60 (100 %)** | 3.5 / **6.3** / 6.3 mm |
| occluded | 20 | 10 (50 %) | 2.7 / 4.2 / 5.1 mm |

거리별로도 갈리지 않는다: **x ≤ 3.0 m 17/17, x > 3.0 m 43/43.** 나머지 가림 10 개는 전부 올바른 거부다(`no_opening_pattern` 5, `opening_width_mismatch` 3, `pocket_occluded` 2).

**그래서 §B 의 125 자세 76 % 는 §D-2 (a) 와 비교할 수치가 아니다.** 카탈로그는 두 개구가 보일 때까지 자세를 재추첨하므로 격자에는 카탈로그가 절대 뽑지 않는 자세가 들어 있다. 같은 모집단에서 재면 **100 %** 이고 로드맵 M2 목표(검출률 ≥ 95 %, 위치 p95 ≤ 20 mm, yaw p95 ≤ 2°)를 **큰 여유로 통과**한다. 참고로 가림 검출률은 v1 의 0.25 보다 높은 0.50 이다.
...
독립 검토 **24회**(v1~v9). 각 회차는 서로의 결론을 보지 않았다.

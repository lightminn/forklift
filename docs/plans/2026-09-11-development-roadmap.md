...
**지원 범위(사용자 확인, 2026-09-17):** EPAL 6과 T11 ×0.6을 모두 지원한다. 아래 전체 작업·A–D 완료 조건은 형상별로 구분하며, 제작 시험체 선택과 분리한다. 형상별 표본 수와 일정은 후속 계획에서 구체화한다. **엔진 역할은 [ADR 0004](../decisions/0004-simulation-engine-and-insertion-depth.md)에서 정했다 — 신규 동적 시뮬레이션은 Isaac Sim 하나로 간다.** 이 추가 요구가 기존 제안 표본 수를 자동 확정하거나 완료 조건을 면제하지 않는다.
...
H4의 실측 기구를 사용해 기존 고정 visual 장면과 별도의 **동적 차체·포크 모델**을 만든다. 엔진은 [ADR 0004](../decisions/0004-simulation-engine-and-insertion-depth.md)가 정한 Isaac Sim 이다(개정 전 문구는 Gazebo 였다). 주행/조향/승강 관절, 구동 제한·feedback, 실제 장착 관계, 차체·포크 충돌 형상을 이 단계에서 검증한다.
...
**완료 조건:** 실제 구동 어댑터와 같은 의미의 명령/피드백으로 **지정 동적 시뮬레이터(ADR 0004: Isaac Sim)** 의 모델과 빈 차체를 각각 시험한다. 직진·곡선·후진·목표 정지, 경로 없음, 장애물 출현, localization/통신 소실의 결과를 보존한다. 차체와 포크의 swept footprint, 속도별 정지 여유를 포함하고 실제 정지 경로가 동작해야 다음 단계로 간다. '장애물 출현'은 위 동적 장애물 시험 표의 다섯 시나리오로 대체하며, 각 시나리오가 표본 수 안에서 기대 판정과 합격 기준을 함께 만족해야 한다. 이 시험은 기능 회귀이며 안전 검증이 아니다.
...
빈 팔레트부터 시작해 실측·합의한 시험 하중으로 확대한다. **지정 동적 시뮬레이터(ADR 0004: Isaac Sim)** 에서는 M4/M5의 구동·포켓 접촉 기준선 위에 동적 팔레트의 질량/마찰·지지 접촉과 하중 상태를 추가로 검증한다. 팔레트를 순간이동시키거나 자동 부착한 경우는 작업 순서 모의시험으로만 분류한다. 실물은 승강·처짐·미끄러짐·하중 상태와 정지 거리를 측정한다. 적재 후 footprint와 속도·회전 제한을 갱신한다. footprint에는 포크 밖으로 나온 화물을 포함한다(회전 시 후미가 도는 범위는 차체 고유 envelope이므로 M4에서 이미 다룬다). 갱신한 값으로 M4의 동적 장애물 시나리오를 **전체 작업 흐름 안에서** 다시 수행한다 — 이송 구간 주행 중에 주입하고, 별도 주행 시험으로 대체하지 않는다. 적재 전후를 분리해 집계하며, 적재 상태의 급정지에는 팔레트 미끄러짐·전도 없음과 지지 접촉 유지를 합격 조건에 더한다. M4가 정의한 '장애물로 멈춘 상태'를 이 상태 흐름에 포함한다.

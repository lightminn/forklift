...
먼저 [CONTRIBUTING.md의 전환 목록](../../CONTRIBUTING.md#10-현재-파일의-적용-계획)에 따라 `src/forklift_core/`, 단위/통합 시험 배치와 패키징을 **별도 변경**으로 옮긴다. 센서 기능 추가와 섞지 않는다. snapshot allowlist·Docker 실행 경로·README·기존 시험 명령도 함께 수정한다. 원격 동작을 포함한 동등성 검사가 끝나면 구조를 고정한다.
...
- [ ] M1 구조 전환의 파일·import·패키징·snapshot 경로를 점검한 별도 계획을 만든 뒤 전환한다. 기존 회귀와 원격 센서 시험으로 동등성을 확인한다.

...
8. **상태 판정(순서):** 부분 구조 후보 생성 → 치수·가시성 검사 → 상태 결정.
   ...
   - **개구별 상부 덱 검사(2026-09-14 신설, 광선 검증보다 먼저).** 두 개구 어느 쪽에도 이 팔레트의 덱이 없으면 → `invalid`(`no_upper_deck`). 한쪽에만 있으면 → `invalid`(`upper_deck_occluded:<side>`, 없는 쪽). **이 검사를 통과하지 못한 패턴은 광선 결과와 무관하게 `valid`가 될 수 없다.**
   - 두 개구부 모두 **열림** → `valid`.
...
1. **가려진 포켓:** 광선 검증으로 가림이 확인되면 `invalid`(`pocket_occluded`)로 낸다(권장). ⚠️ **개정 (2026-09-14): 이 결정은 더 이상 가림 판정의 유일한 경로가 아니다.** 개구별 상부 덱 검사가 **광선 검증보다 먼저** 돌아 `upper_deck_occluded:<side>`를 낼 수 있다. v1 동결 100장면에서 가림 9장면이 `pocket_occluded:<side>`에서 그쪽으로 옮겨갔고, **9장면 모두 광선 검증과 같은 쪽을 지목했다** — 두 증거가 독립적으로 일치한다. 상태는 양쪽 다 `invalid`이므로 이 결정의 취지(가림은 `valid`로 내지 않는다)는 유지된다. 가림 장면 20개의 검출률은 낮게 나오는 것이 정상이며 M3 추적으로 보완한다. 대안: 대칭 추정으로 `valid`를 내되 σ를 키운다(v1에서는 하지 않음 — σ 자체를 `None`으로 두므로 불가).

...
⚠️ **개정 (2026-09-14): 위 덱 점유는 합산이 아니라 개구별로 센다.** 합산 계수는 **한 개구 위에만 덱이 있는 형상**을 팔레트와 구별하지 못한다 — 강한 쪽이 합을 끌고 간다. 그래서 평면 자기 inlier를 `height_m − deck_top_m ≤ z ≤ height_m + plane_inlier_m` **대역**(덱 두께를 넘지 않는다)에서 **개구마다** 세고, **둘 다 문턱을 넘어야** 그 패턴이 `valid`가 될 수 있다. 넘지 못하면 아래 8단계의 새 두 사유로 간다. 합산 계수는 관문에 그대로 남아 있고, 개구별 규칙은 **합산을 이미 통과한 형상에 대해서만** 말한다.
...
8. **상태 판정(순서):** 부분 구조 후보 생성 → 치수·가시성 검사 → 상태 결정.
   ...
   - **개구별 상부 덱 검사(2026-09-14 신설, 폭·광선 검증보다 먼저).** 양쪽 개구의 상부 증거 계수가 모두 문턱 미만이면 → `invalid`(`no_upper_deck`). 한쪽만 미달이면 → `invalid`(`upper_deck_occluded:<side>`, 미달한 쪽). **이 검사를 통과하지 못한 패턴은 폭·광선 결과와 무관하게 `valid`가 될 수 없고, 그 검사들의 사유는 보고되지 않는다**(`pocket_detector.py:672-684`). 문턱은 `upper_band_points or min_band_points`(`:587`)다.
   - 아래 셋은 **코드가 반환하는 순서대로**다(`pocket_detector.py:524-540`).
   - 개구 폭이 prior 범위 밖이거나 두 폭이 20 % 이상 다름 → `invalid`(`opening_width_mismatch`). **먼저 검사한다.**
   - 한쪽 광선의 전방 차단 비율이 한계를 넘거나 뒤가 뚫린 증거가 부족 → `invalid`(`pocket_occluded:<side>` / `pocket_ambiguous:<side>`), 좌·우 순으로 본다. 대칭 추정으로 채우지 않는다.
   - 위 어느 것에도 걸리지 않음 → `valid`.
...
## 7b. 파라미터를 prior 에서 유도한다 (2026-09-14 신설)

절대 길이·절대 개수 다섯이 카탈로그 v1 팔레트에 맞춰 튜닝돼 있었고, **그것이 검출기가 한 규격에 묶인 이유**다. `DetectorParams.derived_for(prior)` 가 그중 넷을 축척하고 하나를 따로 정한다.

### 축척 인자

```
s = prior.opening_height_m / 0.200          # 0.200 m = 카탈로그 v1 의 개구 높이
```

⚠️ **기준점이 v1 인 것이 핵심이다.** s(v1) = 1 이므로 **아래 네 항이 동결값으로 정확히 돌아온다** — v1 회귀가 측정이 아니라 **정의로** 보존된다. 기준점을 EPAL 6 에 두면 v1 의 배율이 2.56 이 되어 **검출이 65/100 → 0/100** 이다(실측).

⚠️ **배율 변수는 세로 치수여야 한다.** inlier 대역의 실패 방식이 "블록 전면과 상판 모서리를 한 평면에 섞는 것" 이라 **세로 방향 혼동**이다. 가로 치수(개구 폭·전체 폭)를 쓰면 T11 × 0.6 에서 배율이 0.825 ~ 0.875 밖에 안 내려가 사각지대가 22 ~ 27 곳 남는다(실측).

### 파라미터별 지수

| 파라미터 | 규칙 | v1 | EPAL 6 (s = 0.390) | T11 × 0.6 (s = 0.225) |
|---|---|---|---|---|
| `plane_inlier_m` | **s** | 20.0 mm | 7.80 mm | 4.50 mm |
| `band_margin_m` | **s** | 10.0 mm | 3.90 mm | 2.25 mm |
| `min_band_points` | **s²** (투영 면적) | 100 | 15 | 5 |
| `min_plane_points` | **s²** | 300 | 46 | 15 |
| `floor_z_m` | **`max(4 mm, deck_bottom_m / 3)`** — s 와 무관 | 16.7 mm | 7.3 mm | 5.0 mm |

**개수가 s² 인 이유:** 개수는 투영 면적이고 면적은 길이의 제곱으로 준다.

**`floor_z_m` 이 s 를 따르지 않는 이유와 상한:** 이 값은 바닥판에서 오고 **`floor_z_m ≤ deck_bottom_m − deck_evidence_tol_m`** 이어야 한다. 넘으면 하부 증거 대역이 잘려 **팔레트·바닥판 없는 구조·바닥까지 내려온 기둥이 동일한 증거를 낸다** — 검출기가 셋을 구별할 수단을 잃는다. **동결값 20 mm 는 EPAL 6(16 mm)·T11(9 mm) 두 실물 형상에서 그 상한을 넘는다.** ⚠️ **나눗수 3 과 4 mm 하한은 유도값이 아니라 dev 튜닝 상수다** — 깊이를 1 mm 로 양자화하는 리그에서 나왔고 실센서에서 재측정해야 한다. 기전이 있는 것은 **분수 형태와 위 상한**뿐이다.

### 유도하지 않는 것 — 각각 측정된 이유가 있다

| 파라미터 | 왜 안 하는가 |
|---|---|
| `cell_m` | 폭 하한이 `opening_width_min_m − 2·cell_m` 이라 **셀을 줄이면 폭 수락 창이 같이 좁아진다** |
| `max_plane_residual_m` | `plane_inlier_m` 에 연동하면 **검출 창의 빗살이 다시 열린다** |
| `deck_evidence_tol_m`·`front_margin_m` | **형상에 대해 측정된 적이 없다** |
| `ransac_iterations` | 개수 문턱이 아니라 표본 성공 확률이다. 별도 항목 |

### v1 회귀

동결 파라미터와 유도 파라미터로 커밋된 v1 100 장면을 각각 재생하면 **`valid` 65/100 로 같고**, 상태·사유가 바뀌는 장면이 **하나**다(`s051`: `upper_deck_occluded:right` → `pocket_occluded:right` — 둘 다 `invalid`, 같은 쪽). 공통으로 `valid` 인 장면의 포켓 최대 이동은 **7.35 mm** 다.

⚠️ **동결 재생 시험은 이 변화를 볼 수 없다** — 동결 YAML 을 읽어 재생하므로 유도 경로를 실행하지 않는다. 그것을 보는 가드는 `tests/unit/perception/test_geometry_family.py` 에 따로 있다.

## 8. 사용자 결정 필요 항목

1. **가려진 포켓:** 광선 검증으로 전방 차단 비율이 한계를 넘으면 `invalid`(`pocket_occluded`)로 낸다(권장). ⚠️ **개정 (2026-09-14): 이 결정은 더 이상 그 사유의 유일한 경로가 아니다.** 개구별 상부 덱 검사가 **광선 검증보다 먼저** 돌아 `upper_deck_occluded:<side>`를 낼 수 있고, 그러면 광선 사유는 보고되지 않는다. v1 동결 100장면에서 9장면이 `pocket_occluded:<side>`에서 그쪽으로 옮겨갔고 **그 9장면에서는 두 검사가 같은 쪽을 지목했다.** ⚠️ **그 일치는 이 모집단의 관측이지 두 검사가 같은 것을 잰다는 근거가 아니다** — 상부 검사는 계수와 문턱만 비교하며 가림을 식별하지 않는다(`pocket-observation.md` 「사유가 보장하지 않는 것」). 서로 다른 쪽에서 동시에 실패하는 구성은 이 9장면이 검증하지 않는다. 상태는 양쪽 다 `invalid`이므로 이 결정의 취지(가림은 `valid`로 내지 않는다)는 유지된다. 가림 장면 20개의 검출률은 낮게 나오는 것이 정상이며 M3 추적으로 보완한다. 대안: 대칭 추정으로 `valid`를 내되 σ를 키운다(v1에서는 하지 않음 — σ 자체를 `None`으로 두므로 불가).

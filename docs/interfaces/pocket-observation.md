...
| 사유 | 상태 | 의미 |
|---|---|---|
| `insufficient_points` | `invalid` | 작업영역 필터를 적용하기 **전** 유한한 입력 점이 `min_plane_points` 미만이다(`pocket_detector.py:629`, 필터는 그 뒤 `:634`) |
...
| **`no_upper_deck`** | `invalid` | **양쪽 개구 모두 상부 증거 계수가 문턱 미만이다.** 덱의 부재를 확정하지 않는다 |
| **`upper_deck_occluded:{side}`** | `invalid` | **한쪽은 문턱을 충족하고 지목한 쪽은 미달했다.** 가림을 확정하지 않으며, 이름의 `occluded`는 원인이 아니라 호환용 식별자다 |
| `exception:{type}` | `invalid` | 검출 중 예외. 역추적은 진단에 들어간다 |

### 사유가 보장하지 않는 것 — 두 경계

1. **보고되는 사유는 대표 실패 하나다.** 다른 실패가 없었다는 뜻도, 지목하지 않은 쪽 포켓이 유효하다는 뜻도 아니다. 검출기는 상부 증거 검사를 폭·광선 검사보다 **우선**해 대표 사유로 보고한다(`pocket_detector.py:672-684`). 상부는 왼쪽이 미달이고 광선은 오른쪽이 차단인 구성에서는 `upper_deck_occluded:left`가 나오며, 오른쪽 차단은 사유에 나타나지 않는다.
2. **사유는 관측 증거의 실패를 나타내고 물리적 원인을 확정하지 않는다.** 판정은 계수와 문턱의 비교뿐이다(`pocket_detector.py:584-591`) — 덱의 부재, 가림, 형상 차이를 구별하는 검사가 아니다. 덱을 실제로 잘라낸 시험 형상도 `upper_deck_occluded`를 낸다(`test_opening_evidence_cases.py:82-108`). 계수 자체는 진단(`:721-722`)에 남으므로, 원인을 알고 싶으면 사유가 아니라 계수를 본다.

⚠️ **`no_upper_deck`의 상태가 `invalid`인 이유는 후보의 검증에 실패했기 때문이다.** 검출기는 **후보를 선택한 뒤** 상부 증거가 문턱에 못 미치면 `invalid`를 내고(`:672-684`), **정상 탐색을 마쳤는데 후보가 하나도 없으면** `no_pallet`을 낸다(`:695-699`). 점 부족과 예외는 그 탐색에 들어가기 전에 갈라지므로 이 구분 밖이다.

⚠️ **"계약이 `no_pallet`을 아무것도 없음으로 정의한다"는 것은 근거가 아니다 — 그런 정의가 없다.** 위 표의 `no_front_plane`·`no_opening_pattern`도 `no_pallet`이며, 어느 것도 공간이 비었다고 말하지 않는다. `no_pallet`은 **이 입력에서 검출 가능한 팔레트 후보를 얻지 못했다**는 뜻이고 물리적 부재를 보증하지 않는다. **"음성 장면에서 `true_negative`로 집계되니까"도 이유가 아니다.** 그것은 지표에 맞춰 계약을 구부리는 것이다.

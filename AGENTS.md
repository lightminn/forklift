...
- **New dynamic simulation goes to Isaac Sim alone** ([ADR 0004](docs/decisions/0004-simulation-engine-and-insertion-depth.md), 2026-09-17). Gazebo is **frozen, not deleted**: its static scene generators, the EPAL and v1 captures and the `forklift_ros` sensor validator stay as a read-only baseline, and no new Gazebo features are written. `tools/scene_rig.py` and MuJoCo stay as CPU-deterministic fixtures, not as simulators. The remote `bench03` Isaac assets are to be brought into the repository; until they are, an Isaac result cannot be reviewed or reproduced from a clone.
- **Insertion depth is a rule, not a constant**: `min(pallet depth x 0.6, 406 mm - 46 mm)`, giving 360 mm for both shapes today for different reasons. The 406 mm carriage limit and the 46 mm reserve are both provisional and move when H0 measures the real carriage; carry the rule, not the number.

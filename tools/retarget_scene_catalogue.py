...
sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "sim/gazebo"))

from build_scene_world import APPROVED_CAMERAS  # noqa: E402
...
def retarget(catalogue: dict, geometry: PalletGeometry, camera: dict | None = None) -> dict:
    # The camera mount is not decided. Everything the visibility block holds --
    # projected corners, whether each opening is in view, how much of one an
    # occluder covers -- is a function of it, so moving the mount means
    # recomputing them, not editing the header.
    camera = camera or catalogue["camera"]
    out_camera = camera
    width = geometry.opening_width_m
    out = dict(catalogue)
    out["camera"] = out_camera
...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        "--camera",
        default=None,
        help="Name of an approved camera mount to retarget onto. The mount is "
        "not decided -- the brief puts the camera on the mast, which moves with "
        "the forks -- and it decides the near limit, so a catalogue captured at "
        "one mount says nothing about another. Default: keep the source's.",
    )
    ...
    camera = None
    if args.camera:
        if args.camera not in APPROVED_CAMERAS:
            raise SystemExit(
                f"unknown camera mount: {args.camera!r}; "
                f"approved: {sorted(APPROVED_CAMERAS)}"
            )
        camera = APPROVED_CAMERAS[args.camera]
    text = yaml.safe_dump(retarget(catalogue, geometry, camera), sort_keys=False)
    header = (
        "# Generated by tools/retarget_scene_catalogue.py from\n"
        f"# {args.catalogue.as_posix()} onto {args.geometry.as_posix()}\n"
        f"# at camera mount {args.camera or '(source)'}.\n"
        "# Poses, categories, splits, lighting, surfaces and distractors are\n"
        "# copied exactly; only what the pallet's shape and the camera mount\n"
        "# decide is recomputed. The mount is not decided -- see APPROVED_CAMERAS.\n"
    )
    if args.output:

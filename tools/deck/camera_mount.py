...
거리는 모두 **카메라에서 팔레트 앞면까지**로 통일한다. 포크 끝은 모델의
...
# (높이 m, 카메라→앞면 최근접 m, 검출 자세 수, 이름)
MOUNTS = [
    (0.27, 1.05, 21, '낮게 설치'),
    (0.50, 1.25, 18, '현재 기준선'),
    (0.90, 1.75, 10, '마스트 상단 · 과제 원문'),
]
...
def build():
    ...
    for z, limit, points, label in MOUNTS:
        ...
        d.text((14, 86), f'가장 가까운 검출 {limit:.2f} m', font=R.F(22), fill=(192, 198, 210))
    ...
    canvas, d, _ = R.titled((W, TOP + body_h + 108), '카메라 높이별 근거리 인식 한계',
                            '거리는 카메라에서 팔레트 앞면까지 · 주황은 카메라가 보는 위아래 범위')
    for i, (cell, gap_mm, points, accent) in enumerate(panels):
        x = i * (PW + GAP)
        canvas.paste(cell, (x, TOP))
        y = TOP + body_h + 12
        d.text((x + 14, y), f'인식이 끊길 때 포크 끝에서 {gap_mm:.0f} mm 남는다',
               font=R.F(23), fill=accent)
        d.text((x + 14, y + 36), f'정면 {SWEEP}자세 중 {points}자세에서 검출',

...
def build():
    ...
    panels = []
    for params, name, accent in ((frozen, '고정값', R.RED), (derived, '팔레트 치수 기반', R.GREEN)):
        ...
        if board:
            d.text((12, max(4, board[0][1] - 30)), '노란 선 = 바닥판 윗면 22 mm',
                   font=R.F(18), fill=(240, 200, 120))
        if line:
            d.text((12, min(PH - 30, line[0][1] + 10)), f'{cut*1000:.1f} mm 아래를 버린다',
    ...
    GAP, TOP = 12, 92
    W = PW * 2 + GAP
    canvas, d, _ = R.titled((W, TOP + PH + BAR + 44), '버리는 구간과 팔레트 바닥판',
                            f'촬영 장면 {SCENE} · 색칠한 띠가 버려지는 구간 · 노란 선이 바닥판 윗면')
